...
const ProjectInfo = () => {
    ...
    const { data: EstInfo, isLoading: isLoadEstInfo } = useQuery(
        ["estinfo", id],
        () => {
            return EstimationServices.getEstimations({ id });
        },
        {
            onSuccess: (data) => {
                console.log("====>hgdfh", data?.data);
            },
            onError: (err) => {
                toast.error(err?.response, { delay: 10 });
            },
    ...
    //     }
    // })
    const handleRedirect = (eid) => {
        window.location.replace("/project/" + id + "/estimateinformation/" + eid + '/estinfo/')
        return
    }
                                ...
                                <div className="project-list-content">
                                    ...
                                    <div className="table-responsive">
                                        <table>
                                            ...
                                            <tbody>
                                                {
                                                    !isLoadEstInfo && EstInfo?.data?.length && EstInfo?.data?.map((each, index) => {
                                                        return <tr key={each?.id}>
                                                            <td>{index + 1}</td>
                                                            <td>{each?.estimate}</td>
                                                            <td>{each?.estimator}</td>
                                                            <td>{moment(each?.date).format('ll')}</td>
                                                            <td>{moment(each?.plan_date).format('ll')}</td>
                                                            <td>{moment(each?.due_date).format('ll')}</td>
                                                            <td>{each?.rv || 0}</td>
                                                            <td>{each?.revision_notes}</td>

                                                            <td className="action">
                                                                {/* <a href={"/project/" + id + "/estimateinformation/" + each?.id + '/estinfo/'}> */}
